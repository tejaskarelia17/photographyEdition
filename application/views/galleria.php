...
#toolbar{display:inline-block; padding:4px 15px; margin:20px 15px; background:#262626 url('<?php echo base_url()?>images\slider\sw_btn_bg.png') repeat-x; -moz-border-radius:8px; -khtml-border-radius:8px; -webkit-border-radius:8px; border-radius:8px; font-family:graublauweb-webfont, Helvetica, Arial, sans-serif; font-size:12px; color:#fff; cursor:pointer;}
#outer_container{position:relative; margin:0; width:700px; padding:0 100px 0 0; z-index:2; background:url('<?php echo base_url()?>images\slider\empty.gif');} /* fucking IE needs a background value to understand hover area */
#customScrollBox{position:relative; overflow:hidden; background:url('<?php echo base_url()?>images\slider\sw_l_bg.png') repeat-y;}
...
#customScrollBox a.thumb_link .selected{position:absolute; top:0; left:0; width:145px; height:91px; background:url('<?php echo base_url()?>images\slider\sw_thumb_selected.png') no-repeat; display:none;}
#dragger_container{position:relative; width:30px; height:580px; float:left; margin:10px 0 0 0; background:url('<?php echo base_url()?>images\slider\sw_dragger_bg.png') repeat-y center;}
...
#arrow_indicator{position:absolute; z-index:1; width:50px; padding:10px; top:50%; margin-top:-25px; left:20px; background:url('<?php echo base_url()?>images\slider\sw_transparent_black_bg.png'); -moz-border-radius:5px; -khtml-border-radius:5px; -webkit-border-radius:5px; border-radius:5px; display:none;}
#nextimage_tip{position:fixed; z-index:1; padding:0 20px; line-height:40px; color:#fff; height:40px; top:50%; margin-top:-20px; right:20px; background:url('<?php echo base_url()?>images\slider\sw_transparent_black_bg.png'); -moz-border-radius:5px; -khtml-border-radius:5px; -webkit-border-radius:5px; border-radius:5px; display:none; font-family:graublauweb-webfont, Helvetica, Arial, sans-serif;}
...
<div id="customScrollBox">
	<div class="container">
    	<div class="content">
            ...
            <p><a href="<?php echo base_url()?>index.php/view/profile/<?php echo $userdata['userData']->id?>"><?php echo $userdata['userData']->username?></a></p>
            <div id="toolbar"></div><div class="clear"></div>
    <?php foreach ($array as $value) :?>
      <a href="<?php echo base_url()."uploads/images/".$value->location?>" class="thumb_link">
          <span class="selected"></span>
          <img width="140" height="140" src="<?php echo base_url()."uploads/images/thumbs/".$value->location?>" title="<?php $value->Title?>" alt="<?php $value->Title?>" class="thumb" />
      </a>        
    <?php endforeach;?>
            <p class="clear"></p>
 			<a href="<?php echo base_url()?>index.php/view/albums/<?php echo $userdata['userData']->id?>">Back</a>
        </div>
	</div>
...
<div id="bg">
<img src="<?php echo base_url()."uploads/images/".$array[0]->location?>" title="<?php $array[0]->Title?>" id="bgimg" />
	<div id="preloader"><img src="<?php echo base_url()?>\images\slider\ajax-loader_dark.gif" width="32" height="32" align="absmiddle" />LOADING...</div>
    <div id="arrow_indicator"><img src="<?php echo base_url()?>images\slider\sw_arrow_indicator.png" width="50" height="50"  /></div>
...
var images=["<?php echo base_url()?>\images\slider\ajax-loader_dark.gif","<?php echo base_url()?>\images\slider\round_custom_scrollbar_bg_over.png"];
...
<script type="text/javascript" src="<?php echo base_url()?>js/galleria/galleria-1.2.9.min.js"></script>
<link type="text/css" rel="stylesheet" href="<?php echo base_url()?>js/galleria/themes/classic/galleria.classic.css">
</body>
</html>