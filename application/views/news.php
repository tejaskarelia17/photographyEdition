...
<div class="nine columns" style="border-right:2px solid #999; overflow:hidden; background-color:transparent;">
	...
	<br>
		...
		<h2 style="font-size:20px;color:#CCC; margin-top:-18px;"><b><?php echo ucwords(strtolower($row->title))?></b></h2>
		</a>
		
		<p class="leadwhite" style="color:#999; width:1000px">
		
		<?php
		echo $this->IPEModel->replace_links(ucwords(strtolower(substr($row->message,0,350))));
		if(strlen($row->message)>351)
			echo "<a href=".base_url()."index.php/news/newsitem/".$row->id."><font color='yellow'>...Read More</font></a>"?></p>
	<br>
<h4 style="font-size:16px;margin-top:-10px; color:grey" align="right">Published on: <?php echo date('d F, Y',  strtotime($row->time))?></h4>
